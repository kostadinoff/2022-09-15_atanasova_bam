...
dat_all %>%
  ...
  geom_vline(aes(xintercept = as.Date('2021-11-11')), color = "red") +
  geom_vline(aes(xintercept = as.Date('2022-03-01'))) +
  geom_vline(aes(xintercept = as.Date('2022-02-01')), color = "red") +
  # За двете оси
...
model_ipw <- glm(
  death ~ factor(variant_dominant) + age + factor(sex),
  data = delta_ipw,
  family=quasipoisson,
  weights = ipw
)
...
#| warning: false
  ...
  round_df(3) %>% 
  mutate(p = "< 0.001") %>% 
  mutate("Променлива" = c("Омикрон", "Възраст", "Мъжки пол" )) %>% 
  knitr::kable()
```
...
rename(predicted = .fitted) %>%
  ...
  geom_rug(aes(x = age), alpha = 0.2, size = 0.4) +
  scale_y_continuous(labels = scales::percent) +
  facet_wrap(sex~variant_dominant)+
  theme(legend.position = "none")+
